...
function Binance_trade(email_text) {
  ...
  if (action === 'BUY') {
    ...
    if (trading[pair]) {
      ...
      //buy_at_market_price(pair)
        ...
        log(chalk.keyword('green')("BUYING 220 LINK, " + pair + " AT MARKET PRICE"))
        ...
        .then( order => {
          ...
          log(logSymbols.success, chalk.green("BUY MARKET ORDER SET "))
  ...
  // #BCE_PAIR_START#BTCUSDT#BCE_PAIR_END#
    ...
    if (trading[pair] && balance[pair]) {
      log(chalk.keyword('red')("SELLING " + balance[pair] + " OF " + pair + " AT MARKET PRICE" ))
      binance_client.order({
        symbol: pair,
        side: 'SELL',
        type: 'MARKET',
      ...
      .then( order => {
        orderId[pair] = order.orderId
        log(logSymbols.success, chalk.red("SELL MARKET ORDER SET "))
        check_market_order(pair, order.orderId)
        balance[pair] = 0
        quantity[pair] = 0
      })
      ...
      log(chalk.keyword('red')("SELLING 220 LINK, " + pair + " AT MARKET PRICE"))
...
function buy_at_market_price(pair) {
  ...
  binance_client.book({ symbol: pair }).then(results => {
    ...
    log(chalk.blue("CURRENT 1ST ASK PRICE : " + buyPrice[pair]))
...
function check_market_order(pair, orderId) {
  ...
  .then( order => {
    if (order.status === "FILLED") {
      log(logSymbols.success, chalk.blue("MARKET ORDER FILLED "))
      return
    }
    else {
      log(logSymbols.warning, chalk.orange("MARKET ORDER NOT YET FILLED "))
